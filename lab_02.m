...
eps = 1*10^(-6);
n = [10,30,200,0];
fid = fopen('C:\Users\user\Desktop\hist.dat', 'w+');
fclose(fid);
fPointer = @lab02_func; %указатель на ф-ю
diary('C:\Users\user\Desktop\hist.dat');

%для каждого количества разбиений создадим стоблец времени
k = 1;
while(k < 4 )
    if k == 1
        t1 = zeros(n(k)+1, 1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%для каждого столбца времени создадим стоблец значений функции
...
divisions(N0,'leftRect',a,b,teta(1),eps,fPointer,d2nArr1);
d2nArr1 = massive;
vpa(d2nArr1,20);
divisions(N0,'rightRect',a,b,teta(1),eps,fPointer,d2nArr2);
d2nArr2 = massive;
divisions(N0,'midRect',a,b,teta(1),eps,fPointer,d2nArr3);
d2nArr3 = massive;
divisions(N0,'trapeze',a,b,teta(1),eps,fPointer,d2nArr4);
d2nArr4 = massive;
divisions(N0,'parabolic',a,b,teta(2),eps,fPointer,d2nArr5);
d2nArr5 = massive;
fprintf('\n');

n(4) = divisions(N0,'leftRect',a,b,teta(1),eps,fPointer,d2nArr6);
d2nArr6  = massive;
...
b = bar(barData);
ax = gca;
ax.XLabel.String = 'N(10,30,200,327680)';
...
b2 = bar(barData2);
ax2 = gca;
ax2.XLabel.String = 'N(10,30,200,327680)';
...
function divs = divisions(N0_, methodToString, a_,b_,teta_,eps_,fPointer_, array)
global massive;
...
% передаем значения двух интегралов од двух разбиений n и 2n
% функция возвращает величину погрешности
...
% эта функция возвращает значения заданной функции
% функция зависит от времени
% время передается как входной параметр
...
% функция возвращает значение интеграла от непрерывной функции
% метод правых прямоугольников  -  rightRect
% метод левых прямоугольников   -  leftRect
% метод средних прямоугольников   -  midRect
% метод трапеций -  trapeze
% метод парабол -  parabolic
...
switch methodToStr
    ...
    case ('parabolic')
        ...
        N = 0.5*n;
         for j = 1 : 1 : (N-1)
         integralSum1 = integralSum1 + fPtr((a+(2*(j-1+1)*((b-a)/n))));
         end
        
       for j = 1 : 1 : n
        integralSum2 = integralSum2 + fPtr((a+((2*j-1)*((b-a)/n))));
       end
       
        integralTempSum = (h/3)*(fPtr(a) + fPtr(a+((0.5*n)*((b-a)/n))) + 2*integralSum1 + 4*integralSum2);
...
% функция возвращает значение интеграла от функции lab02_func
% параметры - время, значения функции lab02_func и строка название метода
% по строке названию метода выбирается , каким способом будет вычислен
% данный интеграл
% метод правых прямоугольников  -  rightRct
% метод левых прямоугольников   -  leftRect
% метод трапеций -  trapeze
